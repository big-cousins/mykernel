...
#define PDE_IDX(addr) ((addr & 0xffc00000) >> 22)				// 获取高10位
#define PTE_IDX(addr) ((addr & 0x003ff000) >> 12)				// 获取中间10位
...
/* 在pf表示的虚拟内存池中申请pg_cnt个虚拟页（连续的），成功则返回虚拟页的起始地址，失败则返回NULL*/
static void* vaddr_get(enum pool_flags pf, uint32_t pg_cnt){
	...
	if(pf == PF_KERNEL) {
		...
		vaddr_start = kernel_pool.vaddr_start + bit_idx_start * PG_SIZE;
	} else {
		// 用户内存池，将来实现用户进程再补充
	}
...
{
	/* 先访问到页表自己 + 再用页目录项pde(页目录内页表的索引)作为pte的索引访问到页表 + \
	再用pte的索引作为页内偏移 */
...
static void page_table_add(void* _vaddr, void* _page_phyaddr)
{
    ...
    *****************************************************************************/
    ...
    if(*pde & 0x00000001) { // 该目录项存在
        ...
        *  访问到pde对应的物理地址，用pde取高20位便可，因为pte基于该pde对应的物理地址内再寻址，把低12位置0便是该pde对应的物理页的起始
        */
        memset((void*)((int)pte & 0xfffff000), 0, PG_SIZE);
        *pte = (page_phyaddr | PG_US_U | PG_US_W | PG_P_1);
    }
}
...
/* 从内核物理内存池中申请pg_cnt个页内存，成功则返回其虚拟地址，失败则返回NULL */
...
/* 用户内存池的位图紧跟在内核内存池位图之后 */
  ...
  /* 位图的数组指向一块未使用的内存,目前定位在内核内存池和用户内存池之外*/
   ...
   kernel_vaddr.vaddr_start = K_HEAP_START;						// 跨过低端1M内存
